Perceptron Building Guide
===========================

This guide will walk you through building the Perceptron device. The only required skill is basic soldering of simple through-hole components to printed circuit boards (PCBs). The outline of the build process is as follows:

1. Obtain the PCBs
2. Obtain all of the other components
3. Solder the components to the PCBs
4. Connect the battery holders and meter and ribbon cable
5. (Optional) build frames and/or front panels to make it look nice

The last step is optional for those who want to make it look nice. You'll need a 3D printer. Without this step, your Perceptron will look something like this:

![Perceptron built without any frame](assets/built_noframe.png)

With a 3D printer, you can make it look like this:

![Perceptron built with frame](assets/built.png)

Either way, it will work the same.
