...
The only friction is a bit of cost. The fabricators generally require a minimum order of 5 boards for each PCB design, and since they're in China, there's significant shipping and duties costs as well. Last I checked from JLCPCB, the cost of 5 boards is about $10-20; multiply that by 2, since there are 2 different PCBs; the shipping cost is about $50; and the duties and taxes are about $15. So the total cost is about $85-105. On the plus side, for that cost, you'll have 5 of each PCB, and they ship pretty quickly. And last I checked from PCBWay, it was a fair bit more expensive, at a total cost of about $135.
...
Though more expensive, PCBWay is slightly easier, since they have a *Shared Projects* feature that allows you to order the boards without having to upload anything (since I've already done that). Still, if you want to save some money, don't be afraid to use JLCPCB. It's not difficult and explained below.

## PCBWay

Here are the links to the 2 PCBs on PCBWay:
- [Input board](https://www.pcbway.com/project/shareproject/Perceptron_af6e14ee.html)
- [Adder board](https://www.pcbway.com/project/shareproject/Perceptron_Adder_Board_01c4b227.html)

For each one, you just need to "Add to cart" and then "Save to Cart", though before you can do that, you'll need to create an account and log in. Before "Save to Cart", you'll see options for 24-hour build time (which in my recent experience is checked by default with no ability to uncheck) and shipping options. You'll also see lots of specifications and other options. No need to change any of those - the defaults are fine - unless you want to order more than 5 of each board. Once you've got both boards in your cart, you can check out in the normal fashion.

## JLCPCB

Here, I'll explain the generic steps that work for pretty much any fabricator, and I'll provide a detailed step-by-step with screenshots for JLCPCB.
...
Now, go to the fabricator's website, [JLCPCB](https://jlcpcb.com/) or whatever other fabricator you may choose, and find the place to "Get instant quote" or similar. That should bring you to a page that has a button to "Add Gerber file" or similar. Click that button and upload either one of the Gerber zip files you just downloaded. The website will upload and process the file (which may take a few minutes), and then it will show you a preview of the PCB with the dimensions automatically detected. For the Input PCB, the dimensions should be 164mm x 86mm. For the Adder PCB, the dimensions should be 122mm x 132mm. You might see a 24-hour build time option at a modest additional cost which you can select. Now you can "Save to cart." There's no need to change any of the specifications or options - the defaults are fine - unless you want to order more than 5 of each board. Now repeat for the other PCB. With both PCBs in your cart, you can proceed to checkout in the standard way.
